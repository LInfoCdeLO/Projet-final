...
public class PanelPont extends JPanel implements MouseListener, MouseMotionListener{
	...
	public void detectionPoint(Point PP){//
		
		if(start==false){
			this.Detecter=false;
			this.n=0;//initialise le compteur a 0
			for(Distance dis: this.listeBarre){//parcourir la liste des distances 
				if(PP.getX()<=dis.getP1().getX()+10 && PP.getX()>=dis.getP1().getX()-10){
					if(PP.getY()<=dis.getP1().getY()+10 && PP.getY()>=dis.getP1().getY()-10){//regarde si le point passe en parametre se trouve a proximite du point P1 de la distance passee en parametre
				...
				if(PP.getX()<=dis.getP2().getX()+10 && PP.getX()>=dis.getP2().getX()-10){
					if(PP.getY()<=dis.getP2().getY()+10 && PP.getY()>=dis.getP2().getY()-10){//regarde si le point passe en parametre se trouve a proximite du point P1 de la distance
						this.Detecter=true;
	...
	public void pointCliquable(Point PP){
		...
		if(start==false){
			...
			for(Distance dis: this.listeBarre){
				if(PP.getX()<=dis.getP1().getX()+10 && PP.getX()>=dis.getP1().getX()-10){
					if(PP.getY()<=dis.getP1().getY()+10 && PP.getY()>=dis.getP1().getY()-10){
						this.cliquable=true;
	...
	public void mouseReleased(MouseEvent e) {
		...
		if(start==false){
			...
			if(build==true && cliquable==true){// si je suis entrain de construire 
				...
				this.detectionPoint(this.pReleased);
