...
public class Torseur implements Comparable<Torseur> {
    ...
    public Arraylist<Torseur> getTorseursExtremites(Arrayliste<Distance> Pont) {
        ...
        for (int i : Pont) { // on parcourt les segments du pont
            ...
            Torseur Torseurextremite = (pont.get(i), TorseurExtremiteAvant.
            X * Math.cos(phi) + TorseurExtremiteAvant.Y * Math.sin(phi), -TorseurExtremiteAvant.X * Math.sin(phi) + TorseurExtremiteAvant.Y * Math.cos(phi), TorseurExtremiteAvant.M, Point
            V);
